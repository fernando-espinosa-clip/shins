    ...
    sass.render(
        ...
        function(err, result) {
            if (err) console.error(err);
                ...
                fs.writeFile(outfile, result.css.toString(), "utf8", function(
                    err
                ) {
...
sassRender(
    path.join(options.root, "source/stylesheets/screen.css.scss"),
    path.join(options.root, "pub/css/screen.css")
);
sassRender(
    path.join(options.root, "source/stylesheets/print.css.scss"),
    path.join(options.root, "pub/css/print.css")
);
